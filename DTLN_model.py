# -*- coding: utf-8 -*-
"""
This File contains everything to train the DTLN model with SI-SDR loss.

Modified version using Scale-Invariant Signal-to-Distortion Ratio (SI-SDR)
as the cost function instead of SNR for less aggressive noise suppression
and better preservation of original speech quality.

Based on original DTLN model by: Nils L. Westhausen (nils.westhausen@uol.de)
Modified: SI-SDR loss implementation
...
class DTLN_model():
    '''
    Class to create and train the DTLN model with SI-SDR loss
    '''
    ...
    def __init__(self):
        ...
        # defining default cost function as SI-SDR instead of SNR
        self.cost_function = self.neg_si_sdr
    ...
    @staticmethod
    def neg_si_sdr(s_estimate, s_true):
        '''
        Static Method defining the negative SI-SDR cost function.
        Scale-Invariant Signal-to-Distortion Ratio (SI-SDR) is computed here.
        This loss function is less aggressive than SNR and better preserves
        the original speech quality while reducing noise.
        
        The SI-SDR is calculated as:
        SI-SDR = 10 * log10(||s_target||^2 / ||e_noise||^2)
        where:
        s_target = <s_estimate, s_true> / ||s_true||^2 * s_true
        e_noise = s_estimate - s_target
        
        Returns negative SI-SDR to be used as a loss (lower is better).
        '''
        
        # Remove mean to make it scale-invariant
        s_true_zm = s_true - tf.reduce_mean(s_true, axis=-1, keepdims=True)
        s_estimate_zm = s_estimate - tf.reduce_mean(s_estimate, axis=-1, keepdims=True)
        
        # Compute the scaling factor (projection)
        # <s_estimate, s_true> / ||s_true||^2
        dot_product = tf.reduce_sum(s_estimate_zm * s_true_zm, axis=-1, keepdims=True)
        s_true_energy = tf.reduce_sum(tf.math.square(s_true_zm), axis=-1, keepdims=True)
        scaling = dot_product / (s_true_energy + 1e-8)
        
        # Compute target signal (scaled true signal)
        s_target = scaling * s_true_zm
        
        # Compute noise signal (residual)
        e_noise = s_estimate_zm - s_target
        
        # Compute SI-SDR
        target_energy = tf.reduce_sum(tf.math.square(s_target), axis=-1, keepdims=True)
        noise_energy = tf.reduce_sum(tf.math.square(e_noise), axis=-1, keepdims=True)
        
        si_sdr = target_energy / (noise_energy + 1e-8)
        
        # Convert to dB scale
        num = tf.math.log(si_sdr + 1e-8)
        denom = tf.math.log(tf.constant(10, dtype=num.dtype))
        si_sdr_db = 10 * (num / denom)
