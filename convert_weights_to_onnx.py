...
Updated to use tf2onnx for better compatibility with modern TensorFlow versions.
...
Version: Updated for TensorFlow 2.x and tf2onnx
...
if __name__ == '__main__':
    ...
    # convert first model
    spec = (tf.TensorSpec((1, 1, dtln_class.blockLen//2+1), tf.float32, name="input_1"),
            tf.TensorSpec((1, dtln_class.numLayer, dtln_class.numUnits, 2), tf.float32, name="input_2"))
    
    onnx_model, _ = tf2onnx.convert.from_keras(model_1, input_signature=spec, opset=13)
    temp_model_file = args.target_folder + '_1.onnx'
    onnx.save(onnx_model, temp_model_file)
    
    # convert second model
    spec = (tf.TensorSpec((1, 1, dtln_class.blockLen), tf.float32, name="input_1"),
            tf.TensorSpec((1, dtln_class.numLayer, dtln_class.numUnits, 2), tf.float32, name="input_2"))
    
    onnx_model, _ = tf2onnx.convert.from_keras(model_2, input_signature=spec, opset=13)
